...
From the above three observations, we can reasonably conclude that we [cannot sacrifice accuracy](/garden/never-sacrifice-test-accuracy/). Accurate tests are the bedrock of trustworthy (and therefore, useful) test suites. Once we maximize accuracy, that leaves us with completeness and speed. Here there is a sliding scale between completeness and speed, and we could potentially rest anywhere on this scale.
