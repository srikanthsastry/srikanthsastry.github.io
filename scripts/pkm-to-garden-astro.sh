...
# pkm-to-garden-astro.sh — Convert PKM files to digital garden entries (Astro)
#
# Astro variant: outputs to src/content/garden/ instead of _garden/.
#
...
#
# Usage:
#   ./scripts/pkm-to-garden-astro.sh [--force] <pkm-file> [<pkm-file> ...]
#
# Examples:
#   ./scripts/pkm-to-garden-astro.sh ~/workspace/brain/thoughts/thought-20260424-directive-gap.md
#   ./scripts/pkm-to-garden-astro.sh --force ~/workspace/brain/sources/source-netflix-context-not-control.md
#   ./scripts/pkm-to-garden-astro.sh ~/workspace/brain/thoughts/thought-20260424-*.md

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
GARDEN_DIR="$REPO_DIR/src/content/garden"
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Convert PKM files to digital garden entries in src/content/garden/"
            echo ""
            echo "Options:"
...
for pkm_file in "${FILES[@]}"; do
    ...
    echo "🌱 Converting: $basename → src/content/garden/${slug}.md"
    ...
    # ── Validate related_notes: only keep slugs that exist in src/content/garden/ ──
    ...
    echo "   ✅ Created src/content/garden/${slug}.md (${maturity})"
...
if [ $created_count -gt 0 ]; then
    echo "Next steps:"
    echo "  1. Review generated files in src/content/garden/"
