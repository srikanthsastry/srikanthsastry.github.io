#!/usr/bin/env bash
# lint-pkm-astro.sh — Validate PKM notes for completeness (Astro variant)
#
# Astro variant: looks for src/content/garden/ and src/content/posts/
# instead of _garden/ and _posts/.
#
# Checks:
...
usage() {
    cat <<'EOF'
Usage: lint-pkm-astro.sh [OPTIONS] [FILE ...]

Lint PKM notes for completeness (Astro variant). When no files are
given, lints every note matching brain/notes/note-*.md.

Options:
  --garden-dir DIR   Path to the digital-garden directory
                     (auto-detected from src/content/garden/ if omitted)
  --help             Show this help and exit

Examples:
  ./brain/scripts/lint-pkm-astro.sh
  ./brain/scripts/lint-pkm-astro.sh brain/notes/note-20260424-confabulation-is-plausible.md
  ./brain/scripts/lint-pkm-astro.sh --garden-dir ~/workspace/srikanthsastry.github.io/src/content/garden/
EOF
    exit 0
}
...
if [[ -z "$GARDEN_DIR" ]]; then
    for candidate in \
        "$WORKSPACE_ROOT/srikanthsastry.github.io/src/content/garden" \
        "$HOME/workspace/srikanthsastry.github.io/src/content/garden" \
        "$WORKSPACE_ROOT/brain/../srikanthsastry.github.io/src/content/garden"; do
        if [[ -d "$candidate" ]]; then
...
if [[ -n "$GARDEN_DIR" ]]; then
    # Garden is at .../src/content/garden, posts at .../src/content/posts
    candidate="$(cd "$(dirname "$GARDEN_DIR")" && pwd)/posts"
    if [[ -d "$candidate" ]]; then
        POSTS_DIR="$candidate"
    fi
fi
