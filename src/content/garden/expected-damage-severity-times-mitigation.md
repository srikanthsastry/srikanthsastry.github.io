...
In practice, neither factor is directly computable. Severity depends on context, downstream dependencies, and failure modes that may not be enumerable. Time to mitigation depends on detection latency and rollback cost, both of which vary by deployment model. Because the ideal metric is uncomputable, you decompose it into assessable proxies: [risk tolerance, feedback latency, and deployment reversibility](/garden/three-dimensions-of-erosion-resistance-allocation/).
